...
function MessagesPage() {
    ...
    async function initialize() {
      ...
      if (!cancelled) {
        setMe({
          ...
          display_name: profile?.display_name || (access.role === "advisor" ? "Advisor" : "Member"),
        });
      }
    }
  ...
  async function loadConversations() {
    ...
    const profileIds = Array.from(
      new Set((rows ?? []).flatMap((row) => [row.teen_id, row.advisor_id].filter(Boolean) as string[])),
    );
  ...
  async function handleRequestCreated(conversationId: string) {
    await loadConversations();
    await navigate({ to: "/messages", search: { c: conversationId, view: undefined } });
  }
      ...
      <Section className="py-8 sm:py-10">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
                {me.is_advisor
                  ...
                  : "Start a new request here, then keep the entire conversation in this inbox."}
              </p>
            </div>
            ...
            {!me.is_advisor && (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full bg-navy px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90"
              >
        ...
        <div className="grid min-h-[650px] gap-5 lg:h-[calc(100vh-11rem)] lg:grid-cols-[340px_1fr]">
              ...
              <div className="grid grid-cols-2 border-b border-border p-2">
                <button
                  ...
                  className={cn(
                    ...
                    tab === "mine" ? "bg-navy text-white" : "text-muted-foreground hover:bg-secondary",
                  )}
                >
                  Assigned to me
                </button>
                <button
                  ...
                  className={cn(
                    "rounded-lg px-3 py-2 text-sm font-semibold",
                    tab === "queue" ? "bg-navy text-white" : "text-muted-foreground hover:bg-secondary",
                  )}
                >
                  Open request queue
                </button>
            ...
            {!me.is_advisor && (
              <div className="border-b border-border p-3">
                <button
                  type="button"
                  onClick={() => setRequestOpen(true)}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-teal/30 bg-teal/5 px-4 py-2.5 text-sm font-semibold text-teal transition hover:bg-teal/10"
                >
                  <Plus className="h-4 w-4" /> New support request
                </button>
              </div>
                ...
                visible.map((conversation) => (
                  <button
                    ...
                    onClick={() =>
                      navigate({
                        to: "/messages",
                        search: { c: conversation.id, view: tab === "queue" ? "queue" : undefined },
                      })
                    }
                    className={cn(
                      "w-full border-b border-border px-4 py-4 text-left transition hover:bg-secondary/60",
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-mesh text-sm font-bold text-white">
                        ...
                        {conversation.unread && (
                          <span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full border-2 border-card bg-teal" />
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 truncate text-xs text-muted-foreground">
                          {conversation.advisor_id === null
                            ? conversation.teen_id === me.id
                              ? "Waiting for a human advisor"
                              : `Unclaimed · ${conversation.topic}`
          ...
          <main className={cn("overflow-hidden rounded-2xl border border-border bg-card flex-col", activeId ? "flex" : "hidden lg:flex")}>
            {!active ? (
              <EmptyThread
                isAdvisor={me.is_advisor}
                queue={tab === "queue"}
                onNewRequest={me.is_advisor ? undefined : () => setRequestOpen(true)}
              />
            ) : activeIsQueueItem ? (
              <QueuePreview
                conversation={active}
                claiming={claiming}
                onClaim={claimActive}
                onBack={() => navigate({ to: "/messages", search: { c: undefined, view: "queue" } })}
              />
            ) : (
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        <button onClick={onBack} className="rounded p-1 hover:bg-secondary lg:hidden">
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div>
          <div className="text-xs font-bold uppercase tracking-widest text-teal">Open advisor queue</div>
      ...
      <div className="flex flex-1 items-center justify-center p-8">
        <div className="max-w-lg text-center">
          ...
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            For privacy, the question itself becomes visible after you claim it. Claiming assigns the conversation to you and tells the user that a human advisor has joined.
          </p>
          <div className="mt-5 rounded-2xl border border-border bg-secondary/40 p-4 text-left text-sm">
            ...
            {conversation.ai_handoff_required && (
              <div className="mt-2 flex items-center gap-2 text-teal">
                <Bot className="h-4 w-4" /> AI activity requires human review
              </div>
            )}
          </div>
          <button
            onClick={onClaim}
            disabled={claiming}
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-mesh px-6 py-3 font-semibold text-white disabled:opacity-50"
          >
      ...
      <div className="mt-1 max-w-sm text-xs">
        {queue
          ...
          : isAdvisor
            ...
            : "Open an existing request, or start a new one without leaving Messages."}
      </div>
      {onNewRequest && (
        <button
          type="button"
          onClick={onNewRequest}
          className="mt-5 inline-flex items-center gap-2 rounded-full bg-navy px-5 py-2.5 text-sm font-semibold text-white"
        >
          <Plus className="h-4 w-4" /> New support request
        </button>
      )}
...
function Thread({
      ...
      .on(
        ...
        (payload) => {
          setMessages((previous) =>
            previous.some((message) => message.id === (payload.new as Message).id)
              ? previous
              : [...previous, payload.new as Message],
          );
          void markRead();
        },
      )
  ...
  async function loadMessages() {
    const { data, error } = await supabase
      .from("messages")
      .select("id, conversation_id, sender_id, sender_kind, body, metadata, created_at")
  ...
  async function markRead() {
    await supabase
      .from("conversation_reads")
      .upsert(
        { conversation_id: conversation.id, user_id: me.id, last_read_at: new Date().toISOString() },
  ...
  return (
    <>
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        <button
          onClick={() => navigate({ to: "/messages", search: { c: undefined, view: undefined } })}
          className="rounded p-1 hover:bg-secondary lg:hidden"
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-mesh text-sm font-semibold text-white">
          {other?.display_name?.[0]?.toUpperCase() ?? (conversation.advisor_id ? "?" : "Q")}
        </div>
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-semibold">{conversation.subject}</div>
          <div className="truncate text-xs text-muted-foreground">
            {conversation.advisor_id ? other?.display_name || "Human advisor" : "Waiting in the human advisor queue"}
          </div>
        </div>
        {other?.calendly_url && (
          <a
            href={other.calendly_url}
            target="_blank"
            rel="noopener noreferrer"
            className="hidden items-center gap-1.5 rounded-full border border-border px-3 py-1.5 text-xs font-semibold sm:inline-flex"
          >
        ...
        <button
          ...
          className="p-1.5 text-muted-foreground hover:text-warn"
        >
      ...
      {conversation.ai_handoff_required && conversation.advisor_id && (
        <div className="flex items-center gap-2 border-b border-teal/20 bg-teal/10 px-4 py-2 text-xs text-navy-deep">
          <Bot className="h-4 w-4 text-teal" /> A limited AI helper replied earlier. The human advisor should review the full context.
        </div>
      )}
      ...
      {conversation.status === "closed" ? (
        <div className="flex items-center justify-center gap-2 border-t border-border p-4 text-sm text-muted-foreground">
          <CheckCircle2 className="h-4 w-4 text-teal" /> This conversation is closed.
        </div>
      ) : (
        <form onSubmit={send} className="border-t border-border p-3">
          <div className="flex gap-2">
            <input
              ...
              maxLength={4000}
              placeholder="Write a message without personal or confidential information…"
              className="flex-1 rounded-full border border-border bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-teal/40"
            />
            <button
              type="submit"
              disabled={sending || askingAi || !body.trim()}
              className="inline-flex items-center justify-center rounded-full bg-mesh px-4 py-2 text-white disabled:opacity-50"
            >
          ...
          {aiAvailable && (
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-teal/5 px-3 py-2">
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                <LockKeyhole className="h-3.5 w-3.5 text-teal" /> No advisor is assigned. The optional AI is deliberately limited and the request remains queued.
              </div>
              <button
                type="button"
                onClick={sendToAi}
                disabled={askingAi || sending || !body.trim()}
                className="inline-flex items-center gap-1.5 rounded-full border border-teal/30 px-3 py-1.5 text-xs font-semibold text-teal disabled:opacity-50"
              >
                {askingAi ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Bot className="h-3.5 w-3.5" />}
                Ask limited AI
              </button>
            </div>
          )}
...
function MessageBubble({ message, mine }: { message: Message; mine: boolean }) {
  if (message.sender_kind === "system") {
    return (
      <div className="flex justify-center">
        <div className="rounded-full border border-border bg-card px-4 py-2 text-center text-[11px] text-muted-foreground">
          {message.body}
        </div>
      </div>
    );
  ...
  return (
    <div className={cn("flex", mine ? "justify-end" : "justify-start")}>
      <div
        className={cn(
          "max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed sm:max-w-[72%]",
          mine
            ? "bg-mesh text-white"
            : ai
              ? "border border-teal/25 bg-teal/10 text-foreground"
              : "border border-border bg-card",
        )}
      >
        {ai && (
          <div className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-teal">
            <Bot className="h-3.5 w-3.5" /> Limited educational AI · not a lawyer
          </div>
        )}
        <div className="whitespace-pre-wrap break-words">{message.body}</div>
        <div className={cn("mt-2 text-right text-[9px]", mine ? "text-white/60" : "text-muted-foreground")}>
  ...
  return (
    <span
      className={cn(
        ...
        ai ? "bg-teal/10 text-teal" : "bg-secondary text-muted-foreground",
      )}
    >
      {children}
    </span>
...
function ReportDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-navy-deep/65 p-4"
      onMouseDown={(event) => {
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl">
        ...
        <textarea
          ...
          className="mt-4 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-teal/40"
        />
