...
function AuthControls({ onNavigate }: { onNavigate?: () => void }) {
  ...
  if (!user) {
    return (
      <Link
        to="/auth"
        onClick={onNavigate}
        className="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full bg-navy px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:opacity-90"
      >
  ...
  if (account?.isAdmin) {
    return (
      <div className="flex shrink-0 items-center gap-1">
        <span className="mr-1 inline-flex items-center gap-1 whitespace-nowrap rounded-full bg-navy px-3 py-1.5 text-xs font-semibold text-white">
          <ShieldCheck className="h-3.5 w-3.5" /> Admin
        </span>
        <AccountLink to="/admin-advisors" label="Advisor applications" icon={UserRoundPlus} onNavigate={onNavigate} />
        <AccountLink to="/admin-competitions" label="Competition administration" icon={Trophy} onNavigate={onNavigate} />
        <AccountLink to="/profile" label="Profile" icon={UserIcon} onNavigate={onNavigate} />
  ...
  if (account?.isAdvisor) {
    return (
      <div className="flex shrink-0 items-center gap-1">
        <span className="mr-1 inline-flex items-center gap-1 whitespace-nowrap rounded-full bg-teal/10 px-3 py-1.5 text-xs font-semibold text-teal">
          <ShieldCheck className="h-3.5 w-3.5" /> Advisor
        </span>
        <AccountLink to="/messages" label="Advisor messages" icon={MessageCircle} onNavigate={onNavigate} />
  ...
  return (
    <div className="flex shrink-0 items-center gap-1">
      <Link
        ...
        onClick={onNavigate}
        className="mr-1 inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-teal/30 bg-teal/5 px-3 py-2 text-xs font-semibold text-teal transition hover:bg-teal/10"
      >
        <UserRoundPlus className="h-3.5 w-3.5" /> {applicationLabel}
      </Link>
      <AccountLink to="/messages" label="Messages" icon={MessageCircle} onNavigate={onNavigate} />
      <AccountLink to="/meetings" label="Meetings" icon={CalendarDays} onNavigate={onNavigate} />
      <AccountLink to="/profile" label="Member profile" icon={UserIcon} onNavigate={onNavigate} />
      <button onClick={handleSignOut} title="Sign out" aria-label="Sign out" className="rounded-md p-2 hover:bg-secondary">
        <LogOut className="h-4 w-4" />
      </button>
    </div>
