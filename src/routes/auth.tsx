...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in or create an account to message advisors and book calls." },
    ],
  }),
  component: AuthPage,
});
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="mt-8 rounded-2xl border border-border bg-card/95 backdrop-blur p-6 shadow-card">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                {mode === "signup" && (
                  <Field icon={UserIcon} placeholder="Display name (e.g. Tara V.)" value={displayName} onChange={setDisplayName} />
                )}
                <Field icon={Mail} type="email" placeholder="you@school.edu" value={email} onChange={setEmail} required />
                <Field icon={Lock} type="password" placeholder="Password (min. 8 characters)" value={password} onChange={setPassword} required minLength={8} />

                {mode === "signup" && (
                  <label className="flex items-start gap-2 text-xs text-muted-foreground pt-1">
                    <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="mt-0.5" />
                    <span>
                      I agree to the <Link to="/advisors" className="text-teal font-semibold underline">community rules</Link> and understand that messages with advisors are educational and not legal advice.
                    </span>
                  </label>
                )}
...
function Field({ icon: Icon, value, onChange, type = "text", placeholder, required, minLength }: {
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.57c2.09-1.93 3.27-4.76 3.27-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.76c-.99.66-2.26 1.06-3.71 1.06-2.86 0-5.28-1.93-6.14-4.53H2.18v2.84A11 11 0 0 0 12 23z"/><path fill="#FBBC05" d="M5.86 14.11A6.6 6.6 0 0 1 5.5 12c0-.73.13-1.44.36-2.11V7.05H2.18A11 11 0 0 0 1 12c0 1.78.43 3.46 1.18 4.95l3.68-2.84z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.07.56 4.21 1.64l3.15-3.15C17.46 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.05l3.68 2.84C6.72 7.31 9.14 5.38 12 5.38z"/></svg>
  );
}
