...
function shortQuote(text: string): string {
  ...
  return trimmed.length <= 220 ? trimmed : trimmed.slice(0, 220).trimEnd();
}
...
export function quotesAreGrounded(text:string,clauses: Array<{quote:string}>){return clauses.every(c=>text.includes(c.quote));}
