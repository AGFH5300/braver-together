...
function EssaySubmissionPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!form.country) {
      toast.error("Select your country or territory.");
      return;
    }
    ...
    return (
      <SiteLayout>
        <Section className="py-24 text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold">The submission portal is unavailable.</h1>
        </Section>
      </SiteLayout>
    );
  }
    ...
    <SiteLayout>
      <div className="relative overflow-hidden bg-hero">
        ...
        <Section className="relative py-16 sm:py-20">
          ...
          <h1 className="mt-4 max-w-3xl text-4xl font-bold text-navy-deep sm:text-5xl">Submit your essay.</h1>
          <p className="mt-4 max-w-2xl text-navy-deep/70">
            Upload your PDF or DOCX, receive your submission reference, and replace your file before the deadline if needed.
          </p>
        </Section>
      </div>
      ...
      <Section className="py-10 sm:py-14">
        ...
        {canSubmit ? (
          <form onSubmit={submit} className="mt-8">
            <div className="mx-auto max-w-4xl space-y-6 rounded-3xl border border-border bg-card p-6 shadow-card sm:p-8">
              <div>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <Field
                  ...
                  onChange={(value) => setForm((current) => ({ ...current, participantName: value }))}
                  required
                  maxLength={100}
                />
                <Field
              ...
              <div className="grid gap-5 sm:grid-cols-[1fr_180px]">
                <Field
                  label="Essay title"
                  value={form.essayTitle}
                  onChange={(value) => setForm((current) => ({ ...current, essayTitle: value }))}
                  required
                  maxLength={180}
                />
                <Field
                    ...
                    {file ? (
                      <>
                        ...
                        <div className="mt-5 flex flex-wrap justify-center gap-2">
                          <label htmlFor="essay-file" className="cursor-pointer rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold hover:border-teal/50">
                            Choose a different file
                          </label>
                          <button type="button" onClick={removeFile} className="inline-flex items-center gap-1.5 rounded-full border border-danger/30 px-4 py-2 text-sm font-semibold text-danger">
                      ...
                      <>
                        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-mesh text-white shadow-glow">
                        ...
                        <label htmlFor="essay-file" className="mt-5 cursor-pointer rounded-full bg-navy px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90">
                          Select file
                        </label>
                      </>
                    )}
              ...
              <div className="space-y-3 rounded-2xl border border-border bg-secondary/40 p-5">
                <Checkbox
                  ...
                  onChange={(value) => setForm((current) => ({ ...current, originalityConfirmed: value }))}
                  label="I confirm that this essay is my own original work and that all sources are acknowledged."
                />
                <Checkbox
                  checked={form.rulesAccepted}
                  onChange={(value) => setForm((current) => ({ ...current, rulesAccepted: value }))}
                  label="I agree to the competition rules and understand that incomplete or ineligible entries may be rejected."
                />
              </div>

              <button
                type="submit"
                disabled={busy || !file || !form.country || !form.rulesAccepted || !form.originalityConfirmed}
          ...
          <div className="mt-8 rounded-3xl border border-dashed border-border bg-card p-10 text-center">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground">
              The submission form will appear here whenever the competition is accepting entries.
            </p>
            <Link to="/competitions" className="mt-5 inline-flex rounded-full border border-border px-5 py-2.5 text-sm font-semibold">
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <span
          className={cn(
            ...
            competition.acceptingSubmissions ? "bg-teal/10 text-teal" : "bg-secondary text-muted-foreground",
          )}
        >
          {statusText}
        </span>
      ...
      {competition.rules_url && (
        <a href={competition.rules_url} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex text-sm font-semibold text-teal hover:underline">
    ...
    <div className="mt-6 rounded-3xl border border-teal/25 bg-teal/5 p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Reference <strong className="text-foreground">{submission.submission_code}</strong> · Revision {submission.revision_number} · Status: {submission.status.replaceAll("_", " ")}
            </p>
            {submission.original_filename && (
              <p className="mt-1 text-sm text-muted-foreground">
                {submission.original_filename} {submission.file_size ? `· ${formatBytes(submission.file_size)}` : ""}
              </p>
            )}
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {submission.original_filename && (
            <button onClick={onDownload} className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold">
              <Download className="h-4 w-4" /> Download
            </button>
          )}
          {canReplace && submission.status !== "withdrawn" && (
            <button onClick={onWithdraw} disabled={withdrawing} className="inline-flex items-center gap-2 rounded-full border border-danger/30 px-4 py-2 text-sm font-semibold text-danger disabled:opacity-50">
              {withdrawing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />} Withdraw
            </button>
          )}
        </div>
      </div>
      {submission.admin_note && (
        <div className="mt-5 rounded-xl border border-border bg-card p-4 text-sm">
          <strong>Review note:</strong> {submission.admin_note}
        </div>
      )}
      {canReplace && (
        <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground">
          <RefreshCw className="h-3.5 w-3.5" /> Submitting another file below replaces this revision while keeping the same reference.
        </div>
      )}
    </div>
  );
...
function stageLabel(stage: string, replacing: boolean) {
  if (stage === "hashing" || stage === "preparing") return "Preparing submission…";
  if (stage === "uploading") return "Uploading essay…";
  if (stage === "verifying") return "Finishing submission…";
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        min={min}
        max={max}
        className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-teal/40"
      />
    </label>
  );
}
