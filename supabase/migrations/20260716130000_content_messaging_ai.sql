...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'profiles_max_active_conversations_check') THEN
    ALTER TABLE public.profiles ADD CONSTRAINT profiles_max_active_conversations_check
      CHECK (max_active_conversations BETWEEN 1 AND 50);
  END IF;
END $$;

ALTER TABLE public.conversations ALTER COLUMN advisor_id DROP NOT NULL;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'messages_sender_kind_check') THEN
    ALTER TABLE public.messages ADD CONSTRAINT messages_sender_kind_check
      CHECK (sender_kind IN ('human', 'ai', 'system'));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'messages_sender_identity_check') THEN
    ALTER TABLE public.messages ADD CONSTRAINT messages_sender_identity_check
      CHECK ((sender_kind = 'human' AND sender_id IS NOT NULL)
...
CREATE INDEX IF NOT EXISTS resource_videos_public_idx ON public.resource_videos(is_published, sort_order, created_at);
CREATE INDEX IF NOT EXISTS news_posts_pub_date_idx ON public.news_posts(pub_date DESC);
CREATE INDEX IF NOT EXISTS profiles_advisor_availability_idx ON public.profiles(is_advisor, is_public, accepting_messages, availability_status);
CREATE INDEX IF NOT EXISTS conversations_queue_idx ON public.conversations(status, advisor_id, created_at);
CREATE INDEX IF NOT EXISTS messages_conversation_created_idx ON public.messages(conversation_id, created_at);

INSERT INTO public.resource_categories (id, label, description, sort_order) VALUES
...
CREATE TRIGGER resource_videos_touch BEFORE UPDATE ON public.resource_videos
...
CREATE POLICY "public read cached news" ON public.news_posts FOR SELECT USING (true);

DROP POLICY IF EXISTS "users read own conversation read state" ON public.conversation_reads;
CREATE POLICY "users read own conversation read state" ON public.conversation_reads FOR SELECT TO authenticated USING (user_id = auth.uid());
DROP POLICY IF EXISTS "users create own conversation read state" ON public.conversation_reads;
CREATE POLICY "users create own conversation read state" ON public.conversation_reads FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid() AND EXISTS (
    SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND (c.teen_id = auth.uid() OR c.advisor_id = auth.uid())
  ));
DROP POLICY IF EXISTS "users update own conversation read state" ON public.conversation_reads;
CREATE POLICY "users update own conversation read state" ON public.conversation_reads FOR UPDATE TO authenticated
...
CREATE POLICY "participants or advisors read conversations" ON public.conversations FOR SELECT TO authenticated USING (
  auth.uid() = teen_id OR auth.uid() = advisor_id OR (
    advisor_id IS NULL AND status = 'open' AND public.has_role(auth.uid(), 'advisor'::public.app_role)
  )
);
CREATE POLICY "teens create support requests" ON public.conversations FOR INSERT TO authenticated WITH CHECK (
  auth.uid() = teen_id AND (
    advisor_id IS NULL OR EXISTS (
      SELECT 1 FROM public.profiles p WHERE p.id = advisor_id AND p.is_advisor = true AND p.is_public = true AND p.accepting_messages = true
    )
  )
);
...
CREATE POLICY "participants read messages" ON public.messages FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND (c.teen_id = auth.uid() OR c.advisor_id = auth.uid()))
);
CREATE POLICY "participants send human messages" ON public.messages FOR INSERT TO authenticated WITH CHECK (
  sender_kind = 'human' AND sender_id = auth.uid() AND EXISTS (
    SELECT 1 FROM public.conversations c WHERE c.id = conversation_id AND c.status = 'open'
