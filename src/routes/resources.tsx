...
import { BrainCircuit, ExternalLink, Lock, MessageCircle, Play, Search, ShieldCheck, Sparkles, Users, Video } from "lucide-react";
...
const categoryIcons = { privacy: Lock, social: Users, contracts: ShieldCheck, safety: ShieldCheck, ai: BrainCircuit, rights: Sparkles } as const;
type LibraryData = { categories: ResourceCategory[]; videos: ResourceVideo[]; commentsConfigured: boolean };

function Resources() {
  const { categories, videos, commentsConfigured } = Route.useLoaderData() as LibraryData;
  ...
  const filtered = useMemo(() => {
    ...
    return videos.filter((video: ResourceVideo) =>
      (activeCategory === "all" || video.category_id === activeCategory) &&
      (!normalized || video.title.toLowerCase().includes(normalized) || video.description.toLowerCase().includes(normalized)),
    );
      ...
      <Section>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between mb-8">
          <div className="flex flex-wrap gap-2">
            <FilterButton active={activeCategory === "all"} onClick={() => setActiveCategory("all")}><Video className="h-3.5 w-3.5" /> All videos</FilterButton>
            {categories.map((category: ResourceCategory) => {
              const Icon = categoryIcons[category.id as keyof typeof categoryIcons] ?? Video;
              return <FilterButton key={category.id} active={activeCategory === category.id} onClick={() => setActiveCategory(category.id)}><Icon className="h-3.5 w-3.5" /> {category.label}</FilterButton>;
            })}
          </div>
          <label className="relative block w-full lg:w-80">
            <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search videos" className="w-full rounded-full border border-border bg-card py-2.5 pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-teal/30" />
          </label>
        </div>

        {videos.length === 0 ? <ComingSoon /> : filtered.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-card p-12 text-center">
            <Search className="mx-auto h-8 w-8 text-muted-foreground/50" /><h2 className="mt-4 text-2xl font-bold">No videos match that search.</h2>
            <button onClick={() => { setQuery(""); setActiveCategory("all"); }} className="mt-4 text-sm font-semibold text-teal hover:underline">Clear filters</button>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filtered.map((video: ResourceVideo) => <VideoCard key={video.id} video={video} onSelect={() => setSelected(video)} />)}
          </div>
        )}
      </Section>

      {selected && <VideoViewer video={selected} commentsConfigured={commentsConfigured} onClose={() => setSelected(null)} />}
    </SiteLayout>
  );
}

function FilterButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button onClick={onClick} className={cn("inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-medium transition", active ? "border-navy bg-navy text-white" : "border-border bg-card text-muted-foreground hover:border-teal/50 hover:text-foreground")}>{children}</button>;
}
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-border bg-card transition hover:border-teal/40 hover:shadow-card">
      <button onClick={onSelect} className="relative block aspect-video w-full overflow-hidden bg-navy text-left">
        ...
        <span className="absolute inset-0 m-auto flex h-14 w-14 items-center justify-center rounded-full bg-white/95 text-navy shadow-lg"><Play className="ml-1 h-5 w-5 fill-current" /></span>
        {video.duration_text && <span className="absolute bottom-3 right-3 rounded bg-black/75 px-2 py-1 text-xs font-semibold text-white">{video.duration_text}</span>}
      </button>
      <div className="p-5"><h2 className="font-display text-xl font-bold leading-tight">{video.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-relaxed text-muted-foreground">{video.description}</p><button onClick={onSelect} className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-teal hover:underline">Watch lesson <Play className="h-3.5 w-3.5" /></button></div>
    </article>
  );
}

function VideoViewer({ video, commentsConfigured, onClose }: { video: ResourceVideo; commentsConfigured: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-navy-deep/85 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="mx-auto my-6 max-w-5xl overflow-hidden rounded-3xl border border-white/10 bg-background shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-border p-5"><div><div className="text-xs font-bold uppercase tracking-widest text-teal">Video lesson</div><h2 className="mt-1 text-2xl font-bold">{video.title}</h2></div><button onClick={onClose} className="rounded-full border border-border px-4 py-2 text-sm font-semibold hover:bg-secondary">Close</button></div>
        <div className="aspect-video bg-black"><iframe src={`https://www.youtube-nocookie.com/embed/${video.youtube_video_id}?rel=0`} title={video.title} className="h-full w-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>
          ...
          <aside className="rounded-2xl border border-border bg-secondary/30 p-4">
            ...
            {comments === null ? (
              ...
              <div className="mt-4 max-h-80 space-y-3 overflow-y-auto pr-1">
                {comments.map((comment: YouTubeComment) => <div key={comment.id} className="rounded-xl border border-border bg-card p-3"><div className="text-xs font-semibold">{comment.author}</div><p className="mt-1 whitespace-pre-line text-xs leading-relaxed text-muted-foreground">{comment.text}</p>{comment.likeCount > 0 && <div className="mt-2 text-[10px] text-muted-foreground">{comment.likeCount} likes</div>}</div>)}
              </div>
            )}
          </aside>
        </div>
